...
struct MenuContentView: View {
    ...
    @State private var launchAtLogin = false  // mirrors LoginItem.isEnabled (synced onAppear)
    @State private var loginError: String?    // inline message if macOS needs login-item approval
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 2) {
            HStack(spacing: 7) {
                ...
                Spacer()
                // Launch at Login lives top-right, opposite the title (only where
                // SMAppService can control it).
            ...
            // Approval message (if macOS needs Login Items approval) sits under
            // the header so it doesn't shove the title around.
            if let loginError {
    ...
    private func setLaunchAtLogin(_ on: Bool) {
        if let reason = LoginItem.setEnabled(on) {
            loginError = reason
            launchAtLogin = LoginItem.isEnabled   // revert to true state
        } else {
            loginError = nil
            launchAtLogin = on
        }
