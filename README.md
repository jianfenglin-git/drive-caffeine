...
Many external hard drives have a **firmware idle timer** (in the enclosure's
...
media library, scrubbing a video timeline, opening a new document. DriveCaffeine 
quietly pokes the drive on a timer so its idle timer never fires and never goes 
to sleep.

## How it works

The idle timer lives in the enclosure bridge chip and resets on I/O traffic
**crossing the bridge** — not on platter motion. Drive Caffeine issues a real
...
v1 done. Published to macOS App Store. 
Validated on real hardware: a sandboxed `F_NOCACHE` read keeps a 45s-timer drive 
awake at 30s intervals, including read-only mounts, and survives
